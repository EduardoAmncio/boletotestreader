...
import confirmIcon from "_assets/icons/iconConfirmButton.svg";
...
export const AuthorizationSheet: React.FC<AuthorizationSheetProps> = ({
  ...
  const generateToken = async () => {
    ...
    try {
      const { url, defaultHeaders } = getBaseRequestData("/AuthorizationToken");
      const data = {
        userId,
        accountId,
      };
  ...
  const onConfirmButtonClick = async () => {
    setState({ loading: true });
    try {
      const { url, defaultHeaders } = getBaseRequestData(
